...
public class ExcelReader {
    ...
    public String readExcelAndConvertToJson(String excelFilePath) {
        ...
        try (FileInputStream fis = new FileInputStream(new File(excelFilePath));
            ...
            for (int i = 0; i < workbook.getNumberOfSheets(); i++) {
                ...
                while (rowIterator.hasNext()) {
                    ...
                    String luName = luNameCell.getStringCellValue().trim();

                    // Create JSON object for all columns including LU Name
                    ObjectNode jsonNode = new ObjectMapper().createObjectNode();
                    jsonNode.put("LU Name", luName); // Add LU Name field

                    // Iterate through selected cells in the row (up to the fifth column)
                    for (int j = 1; j <= 3; j++) { // Iterate only up to the third column as per your example
